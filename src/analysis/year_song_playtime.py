"""year_song_playtime.py

Creates a timeline showing the song with most total play time for each year,
displaying the track name, artist, album information, and total listening time.
"""
...
def run(tracks_df: pd.DataFrame, params: dict[str, Any], output_path: str) -> str:
    ...
    ensure_columns(
        tracks_df, ["Year", "Name", "Artist", "Album", "Total Time", "Play Count"]
    )

    # Drop rows with missing or invalid data
    df = tracks_df.dropna(
        subset=["Year", "Name", "Artist", "Album", "Total Time", "Play Count"]
    ).copy()
    df = df[df["Year"].apply(lambda x: isinstance(x, (int, float)))]
    df["Year"] = df["Year"].astype(int)

    # Convert Total Time and Play Count to numeric, fill missing values with 0
    df["Total Time"] = (
    ...
    # Calculate total play time (duration × plays) in seconds
    # Total Time is in milliseconds, so convert to seconds
    df["Total Play Time"] = (df["Total Time"] / 1000) * df["Play Count"]

    # Find the song with most total play time for each year
    top_songs = (
        df.sort_values(["Year", "Total Play Time"], ascending=[True, False])
        .groupby("Year")
        .first()
        .reset_index()
    )
    ...
    for i, (_, row) in enumerate(top_songs.iterrows()):
        ...
        playtime_text = (
            f"({sec_to_human_readable(int(row['Total Play Time']))} play time)"
        )
    ...
    # Replicate save_plot functionality with 10% additional top space
    ...
    plt.suptitle(
        "Song of the Year Timeline (by Play Time)",
        fontsize=16,
        fontweight="bold",
        fontstyle="italic",
        color="black",
